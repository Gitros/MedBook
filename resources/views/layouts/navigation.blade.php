<nav x-data="{ open: false }" class="bg-white border-b border-gray-100 shadow-sm">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between h-16">
            <div class="flex">
                <div class="shrink-0 flex items-center">
                    <a href="{{ route('dashboard') }}" class="flex items-center gap-2">
                        ...
                        <span class="font-bold text-lg text-gray-800">MedBook</span>
                    </a>
                </div>
            ...
            <div class="hidden sm:flex sm:items-center sm:ms-6">
                <span class="mr-3 text-xs px-2 py-1 rounded
                    @if(auth()->user()->isAdmin()) bg-purple-100 text-purple-800
                    @elseif(auth()->user()->isDoctor()) bg-blue-100 text-blue-800
                    @else bg-green-100 text-green-800 @endif">
                    {{ ['admin'=>'Administrator', 'doctor'=>'Lekarz', 'patient'=>'Pacjent'][auth()->user()->role] }}
                </span>
                ...
                <x-dropdown align="right" width="48">
                    ...
                    <x-slot name="content">
                        <x-dropdown-link :href="route('profile.edit')">{{ __('Konto') }}</x-dropdown-link>
                        @if(auth()->user()->isPatient() && auth()->user()->patient)
                            <x-dropdown-link :href="route('patients.show', auth()->user()->patient)">{{ __('Moje dane') }}</x-dropdown-link>
                        @endif
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            <x-responsive-nav-link :href="route('dashboard')" :active="request()->routeIs('dashboard')">Pulpit</x-responsive-nav-link>
            <x-responsive-nav-link :href="route('doctors.index')" :active="request()->routeIs('doctors.*')">Lekarze</x-responsive-nav-link>
            <x-responsive-nav-link :href="route('appointments.index')" :active="request()->routeIs('appointments.*')">Wizyty</x-responsive-nav-link>
            <x-responsive-nav-link :href="route('specializations.index')" :active="request()->routeIs('specializations.*')">Specjalizacje</x-responsive-nav-link>
            @if(auth()->user()->isAdmin())
                <x-responsive-nav-link :href="route('patients.index')" :active="request()->routeIs('patients.*')">Pacjenci</x-responsive-nav-link>
            @endif
        </div>
        ...
        <div class="pt-4 pb-1 border-t border-gray-200">
            ...
            <div class="mt-3 space-y-1">
                <x-responsive-nav-link :href="route('profile.edit')">Konto</x-responsive-nav-link>
                @if(auth()->user()->isPatient() && auth()->user()->patient)
                    <x-responsive-nav-link :href="route('patients.show', auth()->user()->patient)">Moje dane</x-responsive-nav-link>
                @endif
                <form method="POST" action="{{ route('logout') }}">
                    @csrf
